...
export const SalesAssignment: React.FC = () => {
  const [data, setData] = useState({
    ...
    // PREZZO E CORRISPETTIVO
    prezzo: '', residuoMutuo: '', corrispettivo: '', corrispettivoPerc: '', durataFinoAl: '',
    ...
    <>
      <div className="a4-page" style={{fontSize: '9.5pt', lineHeight: '1.5'}}>
        ...
        <div style={{
          ...
          Incarico Professionale di Valorizzazione Immobiliare
        </div>
        ...
        <div style={{ border: '2px solid #000', padding: '10px', marginBottom: '1.5rem' }}>
          <strong>{data.agNome}</strong>, che accetta, per l'espletamento delle attività di valorizzazione e promozione finalizzate all'individuazione di potenziali acquirenti per l'immobile sito in:
          <br/><br/>
        ...
        <p style={{fontSize: '9pt', textAlign: 'justify', marginBottom: '1rem'}}>
          L'Incaricato si impegna a svolgere attività di marketing, scouting e supporto operativo per favorire la alienazione dell'immobile al prezzo di vendita indicativo di <strong>€ {inp('prezzo', '90px')}</strong>.
        </p>
        ...
        <p style={{marginTop: '0.5rem', textAlign: 'justify'}}>
          ...
          <br/>
          <strong>€ {inp('corrispettivo', '80px')}</strong> (oppure il {inp('corrispettivoPerc', '30px')}% del valore di vendita) oltre IVA.
        </p>
  ...
  return (
    <ContractLayout title="Incarico Valorizzazione" icon={<FileSignature size={20} />} preview={renderPreview()} clientName={data.v1Nome}>
    </ContractLayout>
  );
};
